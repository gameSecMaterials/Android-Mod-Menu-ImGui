...
void ImGui_ImplAndroidGLES2_NewFrame(float width, float height, float millis)
{
    ImGuiIO& io = ImGui::GetIO();
    if (!g_FontTexture){
    ...
    //int w = , h = screenHeight;
    int display_w = width, display_h = height;

    io.DisplaySize = ImVec2((float)display_w, (float)display_h);
    if (display_w > 0 && display_h > 0) {
        io.DisplayFramebufferScale = ImVec2((float) display_w / display_w, (float) display_h / display_h);
    }
